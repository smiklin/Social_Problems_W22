...
```{r read_data}
total_articles <- read.csv("total_articles_month.csv")
subject_articles <- read.csv("subject_articles_month.csv")
...
if ("article_count.csv" %in% files){
  ...
  article_count <- tibble(year = c(2019:2022),month = list(months), n=0) %>%
  unnest(month)
  article_count <- article_count[12:37,]

  #run search by year
  for (i in 1:nrow(article_count)){
    year = article_count$year[i]
    month = article_count$month[i]
    call <- paste0(url,"%28+",query,"+PUBDATETXT%28",month,"+",year,"%29%29&apikey=",key)
    ans <- fromJSON(call)
...
if ("articles_by_subject.csv" %in% files){
  ...
  #create empty data frame
  articles_by_subject <- tibble(year = c(2019:2022),
                             month = list(months), 
                             n=0) %>%
  unnest(month) 

  articles_by_subject <- articles_by_subject[12:37,]
  ...
  # run search by subject and year and month
  for (i in 1:nrow(articles_by_subject)){
    year <- articles_by_subject$year[i]
    month <- articles_by_subject$month[i]
    field <-articles_by_subject$Abb[i]
    
    call <- paste0(url,"%28+",query,"+AND+SUBJAREA+%28+",field,"+%29+AND+PUBDATETXT%28",month,"+",year,"%29%29&apikey=",key)
    ans <- fromJSON(call)
    articles_by_subject$n[i] = as.integer(ans$`search-results`$`opensearch:totalResults`) 
  }
  
  #rename columns
  colnames(articles_by_subject) <- c("year", "month", "n", "field")
