...
```{r read_data}
total_articles_climate <- read.csv("total_articles_Climate_Change.csv")
subject_articles_climate <- read.csv("subject_articles_Climate_Change.csv")
...
article_count <- read.csv("article_count.csv") %>%
  select(-prop, -total)%>%
  left_join(total_articles_climate)%>%
  mutate(prop = n/total)
  
#save file
  write.csv(article_count, "article_count_Climate_Change.csv", row.names = FALSE)
...
articles_by_subject <- read.csv("articles_by_subject.csv")%>%
  select(-prop, -total)%>%
  left_join(subject_articles_climate)%>%
  
  mutate(prop = n/total)
  
#save file
write.csv(articles_by_subject, "articles_by_subject_Climate_Change.csv", row.names = FALSE)
...
NOTE: If running for the first time, make sure to delete "articles_by_asjc.csv" from the folder.
If that file exists, the code will not run and replace it (this is a failsafe)
